...
(function () {
    ...
    gameCommon.getPreviousMapActions = function (jsondata, state) {
        return new Promise(function (resolve, reject) {
            ...
            if (jsondata.showActionsInUserOrder) {
                ...
                for (promptIndex, i = 0; i < state.userChoiceOrder.length; promptIndex = state.userChoiceOrder[++i]) {
                    pushActions(promptIndex);
                }
            } else {
                // Return the actions in the order of the prompts
                for (promptIndex = 0; promptIndex < state.userChoices.length; promptIndex++) {
                    pushActions(promptIndex);
                }
            }
        ...
        return new Promise(function (resolve, reject) {
            ...
            for (i = 0; i < jsondata.promptList.length; i++) {
                ...
                if (jsondata.promptList[i].actionList && jsondata.promptList[i].actionList.length) {
                    ...
                    score = 0;
                    if (typeof state.userChoices[i] == "number" && jsondata.promptList[i].actionList[state.userChoices[i]]) {
                    ...
                    for (j = 0; j < jsondata.promptList[i].actionList.length; j++) {
                        if (jsondata.promptList[i].actionList[j]) {
            ...
            if (somePromptHasAPointValue) {
                ...
                if (!jsondata.layout || !jsondata.layout.hideScoringBreakdown) {
    ...
    gameCommon.pickChoice = function (jsondata, state, promptIndex, choiceIndex) {
        return new Promise(function (resolve, reject) {
            ...
            // If we still don't have a next prompt, check if there is a deprecated "goto" or "endGame" action
            if (nextPromptIndex === null) {
                for (var i = actions.length - 1; i >= 0; i--) {
                    ...
                    } else if (actions[i].name == "endGame") {
                        // Found an endGame action!
                        if (nextPromptIndex === null) {
                            nextPromptIndex = "end";
                        }
                    } else {
                        // Make sure that we don't splice it out
                        continue;
                    }
