            ...
            logIn: function (username, password) {
                return new Promise(function (resolve, reject) {
                    if (!socket) {
                        ...
                        var gameName = document.getElementById("sergis-backend-script").getAttribute("data-gameName");
                        if (!gameName) {
                            reject("Invalid game.");
                            return;
                        }
                        
                        socket.emit("logIn", gameName, username, password, function (userData, _token) {
                            if (userData && _token) {
                                token = _token;
                                resolve(userData);
                            } else {
            ...
            getUser: function () {
                return new Promise(function (resolve, reject) {
                    ...
                    socket.on("connect", function () {
                        ...
                        socket.emit("getUser", gameName, function (userData, _token) {
                            if (userData === false) {
                                reject();
