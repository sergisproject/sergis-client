    ...
    sergis.backend = {
        account: {
            logIn: function (username, password) {
                return new Promise(function (resolve, reject) {
                    if (!socket) {
                        ...
                        var gameUsername = document.getElementById("sergis-backend-script").getAttribute("data-gameUsername"),
                            gameName = document.getElementById("sergis-backend-script").getAttribute("data-gameName");
                        if (!gameUsername || !gameName) {
                            reject("Invalid game.");
                            return;
                        }
                        
                        socket.emit("logIn", gameUsername, gameName, username, password, function (userData, _token) {
                            if (userData && _token) {
                                token = _token;
                                resolve(userData);
                            } else {
            ...
            logOut: function () {
                return new Promise(function (resolve, reject) {
                    if (!socket) {
                        ...
                        socket.emit("logOut", token, function (isResolved) {
            ...
            getUser: function () {
                return new Promise(function (resolve, reject) {
                    // Set up logout URL
                    var logoutUrl = document.getElementById("sergis-backend-script").getAttribute("data-logoutUrl");
                    if (logoutUrl) sergis.logoutUrl = logoutUrl;
                    ...
                    socket.on("connect", function () {
                        ...
                        socket.emit("getUser", gameUsername, gameName, session, function (userData, _token) {
                            if (userData === false) {
                                reject();
