...
(function () {
    ...
    sergis.backend = {
        ...
        game: {
            ...
            getPreviousActions: function () {
                return new Promise(function (resolve, reject) {
                    ...
                    for (i = 0; i < currentPromptIndex - 1; i++) {
                        if (typeof testPrompts[i].chosenAction == "number") {
                            chosenAction = testPrompts[i].chosenAction;
                            for (j = 0; j < testPrompts[i].actions[chosenAction].length; j++) {
                                action = testPrompts[i].actions[chosenAction][j];
            ...
            getActions: function (promptIndex, actionIndex) {
                return new Promise(function (resolve, reject) {
                    // Store what the user chose (so we can access it later using getPreviousActions)
                    testPrompts[promptIndex - 1].chosenAction = actionIndex;
                    resolve(testPrompts[promptIndex - 1].actions[actionIndex]);
                });
            }
        }
    };
