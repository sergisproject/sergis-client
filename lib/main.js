...
sergis.main = {
     ...
     * @param showInstantly {boolean} Whether the loading sign should be shown
     *        right away (instead of using a short timeout) if isLoading==true.
     */
    loading: function (isLoading, showInstantly) {
        if (isLoading) {
            // Make sure it's not already showing or about to show
            if (!sergis.main.loadingVisible && !sergis.main.loadingTimeout) {
                if (showInstantly) {
     ...
     *
     * @param promptIndex {number} The prompt to go to (if it's not the first time).
     */
        ...
        if (sergis.main.promptIndex === null) {
            ...
            if (promptIndex < 1 || promptIndex > sergis.main.promptCount) {
                ...
                sergis.backend.game.getPrompt(promptIndex).then(function (prompt) {
                    ...
                    if (prompt.map) {
                        if (prompt.map.latitude)
                            sergis.main.map.latitude = prompt.map.latitude;
                        if (prompt.map.longitude)
                            sergis.main.map.longitude = prompt.map.longitude;
                        if (prompt.map.zoom)
                            sergis.main.map.zoom = prompt.map.zoom;
                    }
    ...
    /**
     * Handle showing prompt data (i.e. title, content, actions; not map).
     *
    ...
    showPrompt: function (prompt) {
        ...
        if (prompt.answers && prompt.answers.length > 0) {
            ...
            for (i = 0; i < prompt.answers.length; i++) {
                ...
                button.appendChild(sergis.main.renderContent(prompt.answers[i]));
    ...
    /**
     * Handle choosing an answer to the current prompt.
     *
     * @param answerIndex {number} The answer index.
     */
    pickChoice: function (answerIndex) {
        sergis.main.loading(true);
        sergis.backend.game.getActions(sergis.main.promptIndex, answerIndex).then(function (actions) {
