...
sergis.main = {
     ...
     */
    initMap: function () {
        return sergis.frontend.init(document.getElementById("map-container"), sergis.main.map);
    },
    ...
    doActions: function (actions) {
        ...
        for (var i = 0; i < actions.length; i++) {
            if (actions[i].name == "explain") {
                // It's an explanation action
                promises.push(new Promise(function (resolve, reject) {
                    sergis.main.loading(false);
                    // Get the rest of the actions (for later processing)
                    var followingActions = actions.slice(i + 1);
                    sergis.main.showSidebarContent(actions[i].data, function () {
                        // Continue the actions
                        sergis.main.loading(true, true);
                        resolve(sergis.main.doActions(followingActions));
                    });
                }));
                // Don't do the rest of the actions yet
                break;
            } else if (actions[i].frontend && actions[i].frontend == sergis.frontend.name) {
                ...
                if (sergis.frontend.actions.hasOwnProperty(actions[i].name)) {
                    ...
                    // BAD!!! (An error is passed to show the stack trace in the error console.)
                    sergis.error("Invalid action for " + sergis.frontend.name + ": " + actions[i].name, new Error());
                }
            }
        }
