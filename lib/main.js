...
sergis.main = {
    ...
    /**
     * The maximum width ratio of the popup.
     * (Popup max width = full width * this number)
     * @type {number}
     */
    popupMaxWidthRatio: 0.5,
     ...
     * @param {boolean} [isPermanent] - If true, the content will be appended to
     *        the "permanent" section of the popup (i.e. it won't be overwritten
     *        with new content, ever).
     */
    showPopupContent: function (content, isPermanent) {
        // Clear old content
        var contentHolder = document.getElementById(isPermanent ? "map-popup-permanent" : "map-popup"),
            lastChild;
        while (!isPermanent && (lastChild = contentHolder.lastChild)) {
            contentHolder.removeChild(lastChild);
        }
        
        var container = document.getElementById("map-popup-container");
        if (content) {
            // Render new content
            if (content instanceof Element) {
                contentHolder.appendChild(content);
    ...
    /**
     * Adjust the width of the sidebar based on sidebarWidthRatio.
     */
    adjustSidebarWidth: function () {
        ...
        document.getElementById("map-popup-container").style.right = width + "px";
    ...
    adjustSidebarContent: function () {
        var contentTop = 0;
        if (!sergis.main.user.layout || !sergis.main.user.layout.hidePromptTitle) {
            contentTop = document.getElementById("sidebar-title").offsetHeight + "px";
        }
        document.getElementById("sidebar-content").style.top = contentTop;
        document.getElementById("sidebar-content").style.bottom = 
            document.getElementById("sidebar-footer").offsetHeight + "px";
    },
    
    /**
     * Adjust the width and height of the popup container.
     *
     * @return {Promise}
     */
    adjustPopup: function () {
        var popupContainer = document.getElementById("map-popup-container");
        return new Promise(function (resolve, reject) {
            ...
            var mapWidth = document.getElementById("container").getBoundingClientRect().width -
                           parseInt(popupContainer.style.right, 10),
                popupWidth = popupContainer.getBoundingClientRect().width,
                maxWidth = mapWidth * sergis.main.popupMaxWidthRatio;
            if (popupWidth > maxWidth) {
                popupContainer.style.width = Math.floor(maxWidth) + "px";
                // Allow any changes to propogate
                return new Promise(function (resolve, reject) {
                    setTimeout(resolve, 1);
                });
            }
        }).then(function () {
            var sectionHeight = document.getElementById("container").getBoundingClientRect().height,
                popupHeight = popupContainer.getBoundingClientRect().height;
            if (popupHeight >= sectionHeight) {
    ...
    /**
     * Adjust various widths and heights.
     */
    adjust: function () {
        // Make sure the width is good
        sergis.main.adjustSidebarWidth();
        
        // Make sure the height is still good
        sergis.main.adjustSidebarContent();
        
        // Adjust the width/height of the popup if necessary
        sergis.main.adjustPopup();
    },
    ...
    initAfterLogin: function (user) {
        ...
        widthRatio = sergis.main.user.layout && sergis.main.user.layout.defaultPopupMaxWidthRatio;
        if (typeof widthRatio == "number" && widthRatio > 0 && widthRatio <= 1) {
            sergis.main.popupMaxWidthRatio = widthRatio;
        }
        sergis.main.adjust();
    ...
    checkMetadata: function () {
            ...
            select.addEventListener("change", function (event) {
                ...
                sergis.main.checkSkipWarning().then(function (doSkip) {
                    if (doSkip) {
                        ...
                        sergis.main.go(parseInt(select.value, 10), true);
                    } else {
        ...
        // Hide the prompt title bar if needed
        if (sergis.main.user.layout && sergis.main.user.layout.hidePromptTitle) {
     ...
     *        time).
     * @param {boolean} [isJumping=false] - Whether the user got here by
     *        jumping around (which forces reinitialization of the map).
     */
    go: function (promptIndex, isJumping) {
        if (sergis.main.promptIndex === null) {
            ...
            if (promptIndex < 0 || promptIndex >= sergis.main.promptCount) {
                ...
                sergis.backend.game.getPrompt(promptIndex).then(function (prompt) {
                    // If the last prompt's map object says that we have to reinitialize
                    var mustReinitializeMap = sergis.main.map.reinitialize == "after" ||
                                              sergis.main.map.reinitialize == "both";
                    ...
                    // Reinitialize it if we were told to, or if we jumped
                    if (mustReinitializeMap || sergis.main.map.reinitialize == "before" ||
                                               sergis.main.map.reinitialize == "both" ||
                                               sergis.main.user.alwaysReinitializeMap ||
                                               isJumping) {
                        // Either we are set to reinitialize, or the user jumped
                        return sergis.main.reinitMap();
                    } else {
                        // The new map might be different than the old; re-center it
                        return sergis.frontend.centerMap(sergis.main.map, newMapIsDifferent);
                    }
                }).then(function () {
                    // All ready!
                    sergis.main.loading(false);
     ...
     *
     * @param {boolean} isJumping - Whether this function is being called
     *        because the user pressed "skip" (or similar).
     */
    goNext: function (isJumping) {
        var afterSkip = function () {
            ...
            if (sergis.main.promptIndex + 1 >= sergis.main.promptCount) {
                ...
                sergis.main.go(Math.min(sergis.main.promptIndex + 1, sergis.main.promptCount - 1), isJumping);
            }
        };
        if (!isJumping) {
            afterSkip();
     ...
     *
     * @param {boolean} isJumping - Whether this function is being called
     *        because the user pressed "back" (or similar).
     */
    goPrevious: function (isJumping) {
        sergis.main.go(Math.max(sergis.main.promptIndex - 1, 0), isJumping);
    },
