...
if (typeof window.addEventListener == "function" &&
    ...
    (_test_span = document.createElement("span")) &&
    typeof _test_span.getBoundingClientRect == "function") {
    ...
    if (console) {
        ...
        console.log("getBoundingClientRect", typeof _test_span.getBoundingClientRect == "function");
    }
}
...
var sergis = {
    ...
    /**
     * The URL to go to when the user clicks the "Logout" button.
     * If not provided, defaults to just refreshing the page.
     * Sometimes, if a backend has a server-side component, it will set this to
     * the logout URL for the server.
     */
    logoutUrl: null
};
...
sergis.main = {
    ...
    renderContent: function (content, wrapElem) {
        ...
        if (!elem) {
            elem = document.createElement("span");
            elem.appendChild(document.createTextNode(content.value || JSON.stringify(content)));
        }
    ...
    resizerOnMouseMove: function (event) {
        ...
        if (typeof sergis.frontend.mapContainerResized == "function") {
    ...
    initAfterLogin: function (user) {
        // If we don't have a display name or top-right-corner buttons, hide the top-right-corner
        var somethingInTopRightCorner = false;
        // Set up display name
        if (user.displayName) {
            somethingInTopRightCorner = true;
            // Somebody is logged in!
            document.getElementById("main-displayName").textContent = "Logged in as " + user.displayName;
            // Show the Logout button
            document.getElementById("main-logout").style.display = "inline";
        }
        // Set up top-right-corner buttons
        if (user.buttons && user.buttons.length) {
            var buttonContainer = document.getElementById("main-button-container");
            var button;
            for (var i = 0; i < user.buttons.length; i++) {
                if (user.buttons[i]) {
                    somethingInTopRightCorner = true;
                    button = document.createElement("button");
                    button.textContent = user.buttons[i].label;
                    ...
                    })(user.buttons[i].action), false);
                    buttonContainer.appendChild(button);
                }
            }
        }
        // Hide the top-right-corner if needed
        if (!somethingInTopRightCorner) {
    ...
    logOut: function () {
        ...
        sergis.backend.account.logOut().then(function () {
            if (sergis.logoutUrl) {
                ...
                location.href = sergis.logoutUrl;
            } else {
        ...
        return new Promise(function (resolve, reject) {
            sergis.frontend.init(
                ...
                if (toolbarButtons && toolbarButtons.length > 0) {
                    for (var li, i = 0; i < toolbarButtons.length; i++) {
                        li = document.createElement("li");
                        li.appendChild(sergis.main.renderContent(toolbarButtons[i].label));
                        if (toolbarButtons[i].tooltip) li.title = toolbarButtons[i].tooltip;
                        li.addEventListener("click", (function (i) {
                            return (function (event) {
                                sergis.main.loading(true);
                                sergis.frontend.toolbarAction(i).then(function () {
                                    sergis.main.loading(false);
                                }).catch(sergis.error);
                            });
                        })(i), false);
    ...
    reinitMap: function () {
        return new Promise(function (resolve, reject) {
            ...
            sergis.frontend[initFunc](
                document.getElementById("map-container"),
                sergis.main.map
            ).then(function () {
                // Now, get all the previous actions
                sergis.backend.game.getPreviousMapActions().then(function (actions) {
                    // Now, do all the actions
                    sergis.main.doActions(actions).then(function (myRetVal) {
                        // We're finally done
                        resolve();
                    }, reject).catch(sergis.error);
                }, reject).catch(sergis.error);
            ...
            if (promptIndex < 0 || promptIndex >= sergis.main.promptCount) {
                ...
                sergis.backend.game.getPrompt(promptIndex).then(function (prompt) {
                    ...
                    if (promptIndex != oldPromptIndex + 1) {
                        ...
                        sergis.main.reinitMap().then(function () {
                            // All ready
                            sergis.main.loading(false);
                        }).catch(sergis.error);
                    } else {
                        // Just re-center the map
                        sergis.frontend.centerMap(sergis.main.map).then(function () {
                            // All ready
                            sergis.main.loading(false);
                        }).catch(sergis.error);
                    }
    ...
    showPrompt: function (prompt) {
        ...
        document.getElementById("sidebar-title-number").textContent = "" + (sergis.main.promptIndex + 1);
        if (sergis.main.user.jumpingBackAllowed || sergis.main.user.jumpingForwardAllowed) {
    ...
    pickChoice: function (choiceIndex) {
        ...
        sergis.backend.game.getActions(sergis.main.promptIndex, choiceIndex).then(function (actions) {
            ...
            if (!actions || !actions.length) {
                ...
                var nextPrompt = null, last = actions[actions.length - 1];
                // Check the last item, see if it's a "goto"
                if (last.name == "goto" && last.data && last.data.length) {
                    nextPrompt = last.data[0];
    ...
    doActions: function (actions) {
        return new Promise(function (resolve, reject) {
            ...
            // We still have actions to go; process the next one
            ...
            if (action.name == "explain") {
                ...
                sergis.main.showSidebarContent(action.data, function () {
                    ...
                    resolve(sergis.main.doActions(actions.slice(1)));
                });
            } else if (action.frontend && action.frontend == sergis.frontend.name) {
                ...
                if (sergis.frontend.actions.hasOwnProperty(action.name)) {
                    ...
                    sergis.frontend.actions[action.name].apply(sergis.frontend.actions, action.data).then(function () {
                        // Do the remaining actions
                        resolve(sergis.main.doActions(actions.slice(1)));
                    }, reject).catch(sergis.error);
                } else {
                ...
                // We have no idea what this action is; just ignore it.
                // Do the remaining actions
                resolve(sergis.main.doActions(actions.slice(1)));
            }
        });
    ...
    endGame: function () {
        ...
        sergis.backend.game.getGameOverContent().then(function (contents) {
            ...
            document.getElementById("sidebar-title-number").textContent = "";
