...
sergis.main = {
     ...
     * @param {boolean} [dontRemoveContents] - If true, new contents will be
     *        appended to the popup's previous contents instead of overwriting
     *        them.
     */
    showPopupContent: function (content, dontRemoveContents) {
        // Clear old content
        var contentHolder = document.getElementById("map-popup"), lastChild;
        while (!dontRemoveContents && (lastChild = contentHolder.lastChild)) {
            contentHolder.removeChild(lastChild);
        }
