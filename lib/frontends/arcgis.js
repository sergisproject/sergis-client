 ...
 *           object has "name" (a string representing the name that the user
 *           sees), "opacity" (the opacity of the layer, from 0 to 1), and
 *           "urls" (an array of strings with URLs to the layer on ArcGIS
 *           Server). If more than one are provided, then the user is allowed
 *           to switch between the possible layers.
 */
...
(function () {
        ...
        centerMap: function (map) {
            ...
            if (map.frontendInfo && map.frontendInfo.arcgis) {
                ...
                if (map.frontendInfo.arcgis.layers && map.frontendInfo.arcgis.layers.length) {
                    // Load the layers
                    promises.push(loadMapLayers(map.frontendInfo.arcgis.layers));
                } else {
    ...
    /**
     * Current map layers that the user can choose from.
     * @memberof SerGIS_ArcGIS
     * @inner
     */
    var mapLayers = [];
    ...
    /**
     * Load a list of layers for the map.
     *
     ...
     * @param {boolean} [returnDynamicLayers] - Whether to return an array of
     *        the ArcGISDynamicMapServiceLayer objects instead of adding them
     *        to the map right away.
     *
     * @return {Promise.<array>} An array of ArcGISDynamicMapServiceLayer
     *         objects to add to the map (if returnDynamicLayers == true), or
     *         an empty array (if returnDynamicLayers == false).
     */
    function loadMapLayers(layers, returnDynamicLayers) {
        return new Promise(function (resolve, reject) {
            var i, j;
            // Hide all old map layers
            for (i = 0; i < mapLayers.length; i++) {
                for (j = 0; j < mapLayers[i].dynamicLayers; j++) {
            ...
            if (layers && layers.length) {
                var dynamicLayer, dynamicLayers, imageParameters;
                for (i = 0; i < layers.length; i++) {
                    dynamicLayers = [];
                    for (j = 0; j < layers[i].urls.length; j++) {
                        imageParameters = new api.ImageParameters();
                        imageParameters.format = "jpeg"; //set the image type to PNG24, note default is PNG8.
                        dynamicLayer = new api.ArcGISDynamicMapServiceLayer(layers[i].urls[j], {
                            "opacity": typeof layers[i].opacity == "number" ? layers[i].opacity : 1,
                            "imageParameters": imageParameters
                        });
                        // Start it off invisible (unless it's the only one)
                        dynamicLayer.setVisibility(layers.length == 1);
                        // Push it for storage
                        dynamicLayers.push(dynamicLayer);
                        if (returnDynamicLayers) {
                            // They'll add the layer to the map later
                            allDynamicLayers.push(dynamicLayer);
                        } else {
                            ...
                            return (function (event) {
                                ...
                                for (i = 0; i < mapLayers.length; i++) {
                                    for (j = 0; j < mapLayers[i].dynamicLayers.length; j++) {
                                        mapLayers[i].dynamicLayers[j].setVisibility(i == k);
                                    }
                                }
                            });
                        })(k), false);
    ...
    function initMap(mapContainer, map) {
        return new Promise(function (resolve, reject) {
            ...
            if (map.frontendInfo && map.frontendInfo.arcgis) {
                ...
                // Check layers
                if (map.frontendInfo.arcgis.layers && map.frontendInfo.arcgis.layers.length) {
                    mapLayersPromise = loadMapLayers(map.frontendInfo.arcgis.layers, true);
                }
            }
            // Create the map
