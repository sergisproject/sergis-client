...
/**
 * A map of SerGIS_ArcGIS_Unit items to human-readable unit strings, and the
...
(function () {
    ...
    var onDrawEnd = function (event) {
        ...
        drawObject("userDrawing_" + index, geometry, style);
     ...
     * @param {SerGIS_ArcGIS_Unit} unit - The unit of the result.
     */
    var makeUnitStatus = function (number, unit, geometry) {
        ...
        if (sergis_units.area_order.indexOf(unit) != -1) {
            type = "area";
            span.appendChild(document.createTextNode("Area: " + number.toFixed(3) + " "));
        } else {
            type = "length";
            span.appendChild(document.createTextNode("Length: " + number.toFixed(3) + " "));
        }
