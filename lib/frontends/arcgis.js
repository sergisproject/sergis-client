...
(function () {
    ...
    function loadMapLayers(layers) {
        ...
        if (layersWithLegends.length > 0) {
            ...
            // Populate the legend part of the popup
            title = document.createElement("h2");
            title.textContent = "Legend";
            container.appendChild(title);
            
            var div = document.createElement("div");
            var legendId = "id_legend_" + Math.random();
            div.setAttribute("id", legendId);
            container.appendChild(div);
            
            setTimeout(function () {
                var legendDijit = new api.Legend({
                    map: gMap,
