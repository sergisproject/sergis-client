 ...
 *
 * @property {boolean} [hasLegend] - Whether the ArcGIS Server Map Service has
 *           a legend with it.
 */
...
(function () {
    ...
    sergis.frontend = {
        ...
        centerMap: function (map, mapCenterChanged) {
    ...
    /**
     * The esri/digit/Legend object, if we have one.
     * @memberof SerGIS_ArcGIS
     * @inner
     */
    var legend;
     ...
     * @param {Function} callback - A function to call for each item.
     *        The function is called with these 5 parameters:
     *            the group name,
     *            the array item,
        ...
        for (i = 0; i < objects.length; i++) {
            ...
            if (obj[""] && obj[""].length) {
                for (j = 0; j < obj[""].length; j++) {
                    ...
                    if (callback("", obj[""][j], i, j, obj[""].length) === false) {
                        // We're done
                        return;
                    }
                }
                ...
                if (obj.hasOwnProperty(prop) && prop !== "") {
                    if (obj[prop] && obj[prop].length) {
                        for (j = 0; j < obj[prop].length; j++) {
                            // Execute the callback and check its return value
                            if (callback(prop, obj[prop][j], i, j, obj[prop].length) === false) {
                                // We're done
                                return;
                            }
                        }
    ...
    function loadMapLayers(layers) {
        ...
        if (layers && layers.length) {
            ...
            for (var i = 0; i < layers.length; i++) {
                if ((layer = layers[i]) && layer.name && layer.urls.length) {
                    ...
                    // If the layer has a legend, add it
                    if (layer.hasLegend) {
                        (function (layer, newMapLayers) {
                            extendArray(layersWithLegends, newMapLayers.map(function (mapLayer) {
                                return {
                                    layer: mapLayer,
                                    title: layer.name
                                };
        ...
        // and find any other layers with legends
        forEachItemInGroup(mapItems.layersFromActions, function (group, layer) {
            if (layer.toggleable) {
                haveToggleableLayers = true;
            }
            if (layer.hasLegend) {
                extendArray(layersWithLegends, layer.layers.map(function (mapLayer) {
                    return {
                        layer: mapLayer,
                        title: layer.name
                    };
        ...
        updateLayerSelector(haveToggleableLayers);
        
        // Update the legend
        updateLegend(layersWithLegends, haveToggleableLayers);
    }
    
    /**
     * Update the layer selector for the map, if applicable.
     *
     * @param {boolean} haveToggleableLayers - Whether there are toggleable
     *        layers.
     */
    function updateLayerSelector(haveToggleableLayers) {
        if (haveToggleableLayers) {
            // Make a popup container
            ...
            forEachItemInGroup(mapItems.layersFromActions, mapItems.layersFromPrompt,
                               function (group, mapLayer, objectIndex, itemIndex, itemsLength) {
                ...
                // If there is a group w/ more than 1 item, then make a radio; otherwise, a checkbox
                if (group && itemsLength > 1) {
                ...
                if (mapLayer.visible) input.setAttribute("checked", "checked");
                ...
                input.addEventListener("click", function (event) {
                    ...
                    // (necessary because of possible changes in the map legend)
                    setTimeout(function () {
            ...
            // No toggleable layers; hide the popup
            sergis.main.showPopupContent();
        }
    }
    
    /**
     * Update the legend for the map, if applicable.
     *
     * @param {Array} layersWithLegends - The layers with legends.
     * @param {boolean} [haveToggleableLayers] - Whether there are toggleable
     *        layers. (Only used to determine whether to show a hr element.)
     */
    function updateLegend(layersWithLegends, haveToggleableLayers) {
        setTimeout(function () {
            if (legend) {
                // Set visibility based on whether there are layers for the legend
                document.getElementById("map-popup-legend-container").style.display = (layersWithLegends > 0) ? "block" : "none";
                // Update existing legend
                legend.refresh(layersWithLegends);
                // Adjust the width/height of the popup if necessary
                setTimeout(function () {
                    sergis.main.adjustPopup();
                }, 10);
            } else if (layersWithLegends.length > 0) {
                // Make the legend container
                var container = document.createElement("div");
                container.setAttribute("id", "map-popup-legend-container");
                
                var div = document.createElement("div");
                div.setAttribute("id", "map-popup-legend-hr");
                div.appendChild(document.createElement("hr"));
                ...
                div.setAttribute("id", "map-popup-legend-info");
                container.appendChild(div);
                
                sergis.main.showPopupContent(container, true);

                // Make the legend
                legend = new api.Legend({
                    map: gMap,
                    layerInfos: layersWithLegends
                }, "map-popup-legend-info");
                legend.startup();
            ...
            var hr_container = document.getElementById("map-popup-legend-hr");
            if (hr_container) {
                hr_container.style.display = haveToggleableLayers ? "block" : "none";
            }
        }, 1);
    }
            ...
            for (var i = 0; i < layer.urls.length; i++) {
                ...
                switch (layer.type) {
                    ...
                    default:
                        ...
                        // https://developers.arcgis.com/javascript/jsapi/imageparameters-amd.html#format
